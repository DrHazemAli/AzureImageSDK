...
using AzureAISDK.Core.Exceptions;
using AzureAISDK.Inference.Image;
using Microsoft.Extensions.Logging;

namespace AzureAISDK.Core.Services;
...
public class ModelHttpClientService : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(responseContent))
            throw new AzureAIException("Empty response received from server");

        try
        {
            return JsonSerializer.Deserialize<TResponse>(responseContent, _jsonOptions) 
                ?? throw new AzureAIException("Failed to deserialize response");
        }
        catch (JsonException ex)
        {
            throw new AzureAIException("Failed to deserialize response", ex);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt <= model.MaxRetryAttempts; attempt++)
        {
            try
            {
                ...
                request.Headers.Add("User-Agent", "AzureAISDK/1.0.0");
                ...
                {
                    throw new AzureAIException(errorMessage, null, (int)response.StatusCode);
                }

                lastException = new AzureAIException(errorMessage, null, (int)response.StatusCode);
            }
            catch (HttpRequestException ex)
            {
                lastException = new AzureAIException("Network error occurred", ex);
            ...
            {
                lastException = new AzureAIException($"Request timed out after {model.Timeout}", ex);
        ...
        throw lastException ?? new AzureAIException("Request failed after all retry attempts");
    }
